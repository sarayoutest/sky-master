...
>
  ...
  <AppBar
    ...
    sx={{
      ...
      maxWidth: isMobile ? "200px" : "1200px",
      height: 54,
      background: "rgba(217, 217, 217, 0.5)",
  ...
  >
<Toolbar sx={{ px: isMobile ? "8px !important" : "9px !important", display: "flex", justifyContent: "space-between", overflowX: 'hidden',}}>
          {!isMobile && (
            <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
              <Button sx={navBtn} onClick={() => go("/")}>{t('nav.home')}</Button>
              <Button sx={navBtn} onClick={() => go("/about")}>{t('nav.about')}</Button>
              <Button sx={{ ...navBtn, color: "#3b216e", fontWeight: 700 }} endIcon={<KeyboardArrowDownIcon />} onClick={(e) => setServicesAnchor(e.currentTarget)}>{t('nav.services')}</Button>
              <Button sx={navBtn} onClick={() => go("/store")}>{t('nav.store')}</Button>
              <Button sx={navBtn} endIcon={<KeyboardArrowDownIcon />} onClick={(e) => setWorkAnchor(e.currentTarget)}>{t('nav.work')}</Button>
              <Button sx={navBtn} onClick={() => go("/contactX")}>{t('nav.contact')}</Button>
            </Box>
          )}

          <Box sx={{ display: "flex", gap: 1, alignItems: "center", ml: isMobile ? "auto" : "19%" }}>
            {isMobile && <IconButton sx={glassIcon} onClick={() => setDrawerOpen(true)}>
              <MenuIcon sx={{ fontSize: 20, color: "#3b216e" }} /></IconButton>}
  ...
  >
    <ShoppingCartIcon sx={{ fontSize: 25, color: "#fff" }} />
  </Badge>
</IconButton>
 <Box
  onClick={() => {
    ...
    />
      ...
      {/* <AppBar elevation={0}
       ...
       right: isMobile ? 20 : "auto",
        position: "fixed",
       transform: isMobile ? "none" : "translateX(-45%)",
        width: isMobile ? "auto" : "66%", 
        maxWidth: isMobile ? "200px" : "1200px",
         height: 54,
       background: "rgba(217, 217, 217, 0.5)", 
      ...
      <Menu anchorEl={servicesAnchor} open={Boolean(servicesAnchor)} onClose={() => setServicesAnchor(null)} disableScrollLock PaperProps={{ sx: { mt: 2, borderRadius: "24px", minWidth: 550, p: 3, direction: isAr ? 'rtl' : 'ltr' } }}>
